...
## Версия 2.10 (2026-09-10)

* \+ СендерTelegram → Параметры → `senders->telegram->apiBaseUrl`: Новый необязательный параметр. Позволяет задать свой базовый URL Telegram Bot API (например, прокси через Cloudflare Worker) вместо `api.telegram.org` по умолчанию.
* \* Внимание! Требуется PHP >= 7.4 (не тестировался с более ранними версиями).
